/**
 * INFILTRA - Game Logic v0.9.8.8
 * Fusión: Lógica funcional de v0.9.8.5 + mejoras visuales de v0.9.8.7
 */
...
let G = {
    ...
    myRole: null, fullRoles: {}, scores: {}, usedWords: [], currentCategory: null,
    currentSecretWord: null, currentFakeWord: null, starterPlayerId: null, gamePhase: 'home',
    isSpectator: false, votes: {}, votedPlayers: new Set(), voteTargets: {}, timerInterval: null,
    voteTimerInterval: null, refreshInterval: null, voteTimeout: null, spectatorTimerInterval: null,
    soundEnabled: true, screenStack: [], roleRevealed: false, isFirstRound: true
};
...
function init() {
    ...
    console.log('INFILTRA v0.9.8.8 iniciado');
}
...
function initAvatars() {
    ...
    AVATARS.forEach(avatar => {
        ...
        div.onclick = function() {
            ...
            div.classList.add('selected'); updateProfilePreview(); saveProfile();
        };
...
function initFrames() {
    ...
    FRAMES.forEach(frame => {
        ...
        check.className = 'frame-check'; check.textContent = '✓';
...
function initParticles() {
    ...
    for (let i = 0; i < 40; i++) {
        ...
        p.className = 'particle'; p.style.left = Math.random() * 100 + '%'; p.style.top = Math.random() * 100 + '%'; p.style.animationDelay = Math.random() * 5 + 's';
...
function createRoom() {
    ...
    G.channel = generateCode(); G.scores = {}; G.usedWords = []; G.isFirstRound = true; G.gamePhase = 'lobby';
    initPubNub();
}
...
function joinRoom() {
    ...
    saveProfile(); G.isHost = false; G.channel = code; G.gamePhase = 'lobby';
    initPubNub();
}
...
function onStatus(status) {
    if (status.category === 'PNConnectedCategory') {
        ...
        if (G.isHost) { document.getElementById('btn-distribute').style.display = 'block'; generateQR(); setTimeout(publishConfig, 300); }
...
function onMessage(event) {
    const msg = event.message; const sender = event.publisher;
    switch (msg.type) {
        case 'config':
            G.maxPlayers = Math.min(msg.maxPlayers, 10); G.roundTime = msg.roundTime; G.hostId = msg.hostId; G.isHost = (G.myId === G.hostId);
            if (msg.usedWords) G.usedWords = msg.usedWords; if (msg.scores) G.scores = msg.scores; renderPlayerList(); break;
        case 'player_state':
            G.players[sender] = { name: msg.name, avatar: msg.avatar, frame: msg.frame };
            if (G.scores[sender] === undefined) G.scores[sender] = 0; renderPlayerList(); break;
        ...
        case 'kick_player':
            ...
            else { delete G.players[msg.targetId]; delete G.scores[msg.targetId]; renderPlayerList(); } break;
...
function handleFullSync(msg) {
    ...
    G.usedWords = msg.usedWords || []; G.scores = msg.scores || {};
...
function refreshPlayers() {
    ...
    G.pubnub.hereNow({ channels: [G.channel], includeState: true }, function(status, response) {
        if (response && response.channels && response.channels[G.channel]) {
            ...
            occupants.forEach(o => {
                if (!G.players[o.uuid]) G.players[o.uuid] = { name: o.state?.name || o.uuid.substring(0, 8), avatar: o.state?.avatar || 'avatar-11', frame: o.state?.frame || 'fr-basic' };
...
function renderPlayerList() {
    ...
    list.innerHTML = playerIds.map(id => {
        const p = G.players[id]; const isMe = id === G.myId; const isHostPlayer = id === G.hostId; const score = G.scores[id] || 0;
        const kickBtn = (G.isHost && !isMe && (G.gamePhase === 'lobby' || G.gamePhase === 'home')) ? '<button class="btn-kick" onclick="kickPlayer(\'' + id + '\')" title="Expulsar"><img src="' + ICONS.close + '" alt="X"></button>' : '';
...
function kickPlayer(playerId) {
    ...
    if (confirm('¿Expulsar a ' + playerName + '?')) G.pubnub.publish({ channel: G.channel, message: { type: 'kick_player', targetId: playerId, targetName: playerName } });
}
...
function distributeRoles() {
    ...
    let roles = {}; let pool = [...playerIds];
    G.impostors = []; G.charlatans = []; G.citizens = [];
    for (let i = 0; i < numImp && pool.length; i++) { const idx = Math.floor(Math.random() * pool.length); const id = pool.splice(idx, 1)[0]; roles[id] = { role: 'INFILTRADO', icon: ICONS.impostor, word: 'Categoría: ' + wordData.category }; G.impostors.push(id); }
    for (let i = 0; i < numChar && pool.length; i++) { const idx = Math.floor(Math.random() * pool.length); const id = pool.splice(idx, 1)[0]; roles[id] = { role: 'CHARLATÁN', icon: ICONS.charlatan, word: wordData.fakeWord }; G.charlatans.push(id); }
...
function handleAssign(msg) {
    ...
    if (G.isFirstRound) {
        ...
        roleTitle.textContent = 'SECRETO'; roleWord.textContent = '???'; roleInst.textContent = 'Toca la carta para revelar';
    } else {
        G.roleRevealed = true;
        ...
        roleTitle.textContent = G.myRole.role; roleWord.textContent = G.myRole.word; roleInst.textContent = 'Tu rol (ya revelado)';
    }
...
function handleStartRound(msg) {
    ...
    if (G.isSpectator) { document.getElementById('spectator-status').textContent = starterName + ' inicia!'; setTimeout(function() { startSpectatorTimer(msg.time); }, 2000); return; }
    const starterInfo = document.getElementById('starter-info');
    if (starterInfo) { starterInfo.textContent = '¡' + starterName + ' inicia!'; starterInfo.style.display = 'block'; }
    setTimeout(function() { if (starterInfo) starterInfo.style.display = 'none'; startTimer(msg.time); }, 2000);
}
...
function startTimer(duration) {
    ...
    timer.style.display = 'block'; timer.classList.remove('warning');
...
function renderVotingList() {
    ...
    list.querySelectorAll('.btn-vote').forEach(btn => { btn.onclick = function() { sendVote(btn.dataset.target, btn); }; });
}
...
function startVoteTimer(seconds) {
    ...
    G.voteTimerInterval = setInterval(function() { remaining--; if (remaining < 0) { clearInterval(G.voteTimerInterval); return; } display.textContent = '00:' + remaining.toString().padStart(2, '0'); }, 1000);
}
...
function publishResults() {
    if (!G.pubnub) return;
    clearAllTimers();
    let maxVotes = 0; let mostVoted = [];
    Object.entries(G.votes).forEach(function([id, count]) { if (count > maxVotes) { maxVotes = count; mostVoted = [id]; } else if (count === maxVotes) mostVoted.push(id); });
    const isTie = mostVoted.length > 1 || maxVotes === 0;
    let eliminatedId = null; let eliminatedRole = null;
    if (!isTie) {
        eliminatedId = mostVoted[0]; G.eliminated.push(eliminatedId); G.activePlayers = G.activePlayers.filter(id => id !== eliminatedId);
...
function showResults(msg) {
    clearAllTimers(); G.votes = msg.votes; G.scores = msg.scores || G.scores; G.activePlayers = msg.activePlayers; G.impostors = msg.impostors;
    ...
    if (msg.eliminatedId === G.myId) {
        ...
        if (G.isHost) { document.getElementById('btn-spectator-next').style.display = 'block'; document.getElementById('btn-spectator-lobby').style.display = 'block'; }
        return;
    }
    if (G.isSpectator) {
    ...
    if (resultsList) {
        ...
        resultsList.innerHTML = voteEntries.map(function([id, count]) { return '<div class="result-item"><div class="result-header"><span class="result-name">' + (G.players[id]?.name || id) + '</span><span class="result-votes">' + count + ' votos</span></div><div class="result-bar"><div class="result-bar-fill" style="width:' + (count / maxVotes * 100) + '%"></div></div></div>'; }).join('');
    }
    ...
    if (elimBox) {
        if (msg.isTie) elimBox.innerHTML = '<div class="eliminated-icon"><img src="' + ICONS.tie + '" alt="" class="eliminated-icon-img"></div><div class="eliminated-name">EMPATE</div><div class="eliminated-role">Nadie eliminado</div>';
            ...
            elimBox.innerHTML = '<div class="eliminated-icon"><img src="' + iconSrc + '" alt="" class="eliminated-icon-img"></div><div class="eliminated-name">' + msg.eliminatedName + '</div><div class="eliminated-role">Era ' + msg.eliminatedRole + '</div>';
        }
    }
...
function handleNextRound(msg) {
    ...
    if (G.isSpectator) {
        ...
        showScreen('screen-spectator'); updateSpectatorRoles(); return;
    }
    const card = document.getElementById('role-card');
    const btnStart = document.getElementById('btn-start-round');
    const btnSkip = document.getElementById('btn-skip-word');
...
function handleGameOver(msg) {
    ...
    scoresList.innerHTML = sorted.map(function([id, score], idx) {
        const p = G.players[id]; const role = G.fullRoles[id];
        let medalHtml = '';
...
function backToLobby() {
    if (G.isHost && G.pubnub) G.pubnub.publish({ channel: G.channel, message: { type: 'back_to_lobby', scores: G.scores, hostId: G.hostId, usedWords: G.usedWords } });
    ...
    G.refreshInterval = setInterval(refreshPlayers, 3000); refreshPlayers();
}

function handleBackToLobby(msg) {
    clearAllTimers(); G.scores = msg.scores || G.scores; G.hostId = msg.hostId || G.hostId; G.isHost = (G.myId === G.hostId); G.usedWords = msg.usedWords || G.usedWords;
    resetGameState(); showScreen('screen-lobby');
    const btn = document.getElementById('btn-distribute');
    if (btn) btn.style.display = G.isHost ? 'block' : 'none';
    G.refreshInterval = setInterval(refreshPlayers, 3000); refreshPlayers();
...
function resetGameState() {
    clearAllTimers(); G.gamePhase = 'lobby'; G.isSpectator = false; G.activePlayers = []; G.eliminated = [];
    G.impostors = []; G.charlatans = []; G.citizens = []; G.myRole = null; G.fullRoles = {};
    G.votes = {}; G.votedPlayers = new Set(); G.voteTargets = {}; G.roleRevealed = false; G.starterPlayerId = null; G.isFirstRound = true;
}
...
function updateSpectatorRoles() {
    ...
    list.innerHTML = Object.entries(G.fullRoles).map(function([id, role]) {
        const p = G.players[id]; const isActive = G.activePlayers.includes(id); const statusIcon = isActive ? ICONS.active : ICONS.eliminated;
...
function updateSpectatorVotes() {
    ...
    list.innerHTML = G.activePlayers.map(function(id) {
        const p = G.players[id]; const votes = G.votes[id] || 0; const hasVoted = G.votedPlayers.has(id); const statusIcon = hasVoted ? ICONS.voted : ICONS.pending;
...
function exitGame() {
    clearAllTimers(); clearInterval(G.refreshInterval);
    if (G.isHost && G.pubnub) G.pubnub.publish({ channel: G.channel, message: { type: 'host_disconnect' } });
    if (G.pubnub) { G.pubnub.unsubscribeAll(); G.pubnub = null; }
    G.channel = null; G.isHost = false; G.hostId = null; G.players = {}; G.scores = {}; G.usedWords = [];
